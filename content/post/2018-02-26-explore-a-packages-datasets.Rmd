...
head(sample_list$topo)
 ...
 data(package = c("MASS","lattice"))$results[,c("Package","Item","Title")] %>% as.data.frame(stringsAsFactors = FALSE) %>% View()
 
 
  data(package = .packages(TRUE) )$results[,c("Package","Item","Title")] %>% as.data.frame(stringsAsFactors = FALSE) %>% View()
