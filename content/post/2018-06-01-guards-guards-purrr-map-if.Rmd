...
In the example below I use `map_if` to test each item in the `params` list. My guard (predicate) function is `lubridate::is.Date`.  If the guard returns `TRUE` for an item, that item gets passed to the `format` function with the additional `format = "%d %B %Y"` argument. Sweet! `r blogdown::shortcode("emoji", "tada")`
